    ...
    const auto push_number = [
        ...
        if (num.has_value())
            tokens.push_back(std::to_string(*num) +
                (is_dev_frac ?
                    (std::string(".") + std::to_string(decimal_fraction)) :
                    std::string()));
        num.reset();
        decimal_fraction = 0;
        is_dev_frac = false;
            ...
            if (isOp({ ch })) {
                ...
                if (!is_dev_frac)
                    *num = *num * 10 + static_cast<int>(ch - '0');
                    ...
                    decimal_fraction = decimal_fraction * 10 +
...
std::string infix2postfix(std::string infix) {
    ...
    std::for_each(std::begin(tokens), std::end(tokens),
        [&opStack, &postfix](const std::string& token) {
            if (isOp(token)) {
                if (token == ")") {
                    while (opStack.get() != '(') {
                        postfix += opStack.get();
                    ...
                    if (priorityCurrent == 0 ||
                        opStack.isEmpty() ||
                        priorityCurrent > priority(opStack.get())) {
                        opStack.push(token[0]);
                    } else {
                        int priorityPrev;
                        while (!opStack.isEmpty() &&
                            (priorityPrev = priority(opStack.get()))
                            >= priorityCurrent) {
                            postfix += opStack.get();
                            postfix += ' ';
                            opStack.pop();
